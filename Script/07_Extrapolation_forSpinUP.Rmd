...
# new toKeep
toKeep<-sort(unique(effort$LME_FAO))
...
if(df_kappa == 6){
  name = "Output/reconstructed_effort_for_spinup_totLME2_JeromeComments_kappa6_1841_fixedValues_kappaLME31_EEZ_addFAO.pdf" # this version uses EEZ instead of administrative country # this version uses FAO regions instead of LME 0
}else if (df_kappa == 3){
  name = "Output/reconstructed_effort_for_spinup_totLME2_JeromeComments_kappa3_1841_fixedValues_kappaLME31_EEZ_addFAO.pdf"
}
...
effort_spread<-function(historical_effort, recent_effort, toKeep, reference){
  ...
  ####### WARNING this is where it get tricky with the new organisation of LME 0 if you are considering EEZ and FAO area only. What happens with area that is outside EEZ and LME0+FAO classification and withing an LME? should you check that the sum of effort when using different grouping is always the same??? of course need to group_by fao_area too below!
...
#### WRNING, again, this is where it can get tricky - should you run this at LME level as before or using LME_FAO column? 
