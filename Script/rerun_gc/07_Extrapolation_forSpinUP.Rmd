...
dir_new <- file.path("/home/ubuntu/gem/private/users/gclawson")
...
effort <- fread(file.path(dir_new, "effort_histsoc_1950_2017_EEZ_addFAO.csv"), data.table = FALSE) 
...
catch<- fread(file.path(dir_new, "catch_histsoc_1869_2017_EEZ_addFAO.csv"), data.table = FALSE)
...
# for effort 
effort <- effort %>% 
  mutate(LME_FAO = ifelse(LME == "", paste0("LME0_FAO_", fao_area), paste0("LME_", LME)))
...
df_kappa = 6 
  ...
  kappa_values <- kappa_values %>% 
    mutate(kappa = case_when(lme %in% c("LME_Gulf of Mexico","LME_Humboldt Current","LME_Greenland Sea","LME_East Siberian Sea",
                                        "LME_Laptev Sea","LME_Antarctica","LME_Southeast Australian Shelf","LME_Northern Bering - Chukchi Seas") ~ 5,
                             lme %in% c("LME_Scotian Shelf","LME_Benguela Current","LME_Somali Coastal Current","LME_Canadian High Arctic - North Greenland") ~ 4,
                             lme %in% c("LME_Northeast Australian Shelf") ~ 3,
...
plot_reconstructed_effort_LME_war_ref$`LME_Canadian High Arctic - North Greenland`

# show Antarctica - need to be 0 before 1950 - it is zero or close to zero in both kappa cases  
plot_reconstructed_effort_LME_war_ref$`LME_Antarctica`

# show north sea
plot_reconstructed_effort_LME_war_ref$`LME_North Sea`
...
trial<-reconstructed_effort_LME_war_ref$`LME_Central Arctic` 
trial1<-reconstructed_effort_LME_war_ref$`LME_Aleutian Islands`
...
plot_LME64<-ggplot(data = trial3_toPlot, aes(x = Year, y = Value, group = Data, color = Data)) + 
  ggtitle(paste("LME Central Arctic", sep = " "))+
...
reconstructed_effort_LME_war_ref$`LME_Central Arctic`<-trial3
plot_reconstructed_effort_LME_war_ref$`LME_Central Arctic`<-plot_LME64
...
### replace name above with something shorter...  
  name = "Output/rerun/reconstructed_effort_for_spinup_EEZ_addFAO.pdf" # this version uses EEZ instead of administrative country # this version uses FAO regions instead of LME 0
...
save(reconstructed_effort_LME_war_ref, plot_reconstructed_effort_LME_war_ref, data_for_plot_for_FAOreport, file = here("Data/temp_effort.RData"))
