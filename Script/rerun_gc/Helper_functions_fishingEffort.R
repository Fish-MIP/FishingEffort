...
join_effort_data <- function(this_file_name){
  ...
  # trial 
  # this_file_name = "mapped_1950_APW_156.csv"
  
  this_source_path <- file.path("/home/ubuntu/gem/private/users/yannickr/effort_mapped_bycountry", this_file_name)
  
  # this version considers EEZ aggregation using EEZ FAOname instead of the administrative country name. 
  this_destination_path <- paste0("/home/ubuntu/gem/private/users/gclawson/effort_mapped_by_country_aggregated_EEZ/", "aggregated_EEZ_", this_file_name)
  
  # if(!file.exists(this_destination_path)){  # comment out if you want to skip files which are already done running
  ...
  these_aggregated_data <-
    these_data %>%
    left_join(LMEs, by=c("x", "y")) %>%
    left_join(EEZ, by=c("x", "y")) %>%
    left_join(FAO_regions, by=c("x", "y")) %>%
    # as.data.frame()
    group_by(eez_name, lme_name, fao_region_name, SAUP, Gear, FGroup, Sector) %>%
  ...
  ## seeing some weirdness with the black sea... probably because we classify that as land in some areas? 
...
effort_extrapolate<-function(toKeep, kappa, war_param, war_decision, reference){
  ...
  if((year[[1]]<=1918 | year[[1]]<=1945) & war_param == "exclude"){ # OR if warToRemove exist? 
    ...
    # option 2 no war only if data include both wars   
  }else if(year[[1]]<=1918 & war_param == "partially-exclude"){
    df<-filter(df, !Year %in% warToRemove)
  }else{
    # option 3 yes war  
    df<-df
  ...
  if(var == "catch_tot"){
    ...
    # WARNING - this is to fix "LME0_FAO88" where effort data is missing before 2004 
    ...
    # (relate to LME0_FAO21 and corrected above when effort is calculated using catches)
  ...
  ## LME 12 - catch based prediction 
  # # how it is given {} around else: 
  # 1  1950 NomActive 60007404. Effort (nominal, DkW)
  # 2  1950 catch_tot   165558. Catch (tonnes)       
  # 3  1950 pred         61453. Catch (tonnes)       
  # 
  # # how it is given () around else: SAME so OK! 
  # Year Type          Value group                
  # <dbl> <chr>     <dbl[1d]> <chr>                
  # 1  1950 NomActive 60007404. Effort (nominal, DkW)
  # 2  1950 catch_tot   165558. Catch (tonnes)       
  # 3  1950 pred         61453. Catch (tonnes)       
  #
  # # how it is without else{} 
  # both lines above apply 
  # so "pred" = Effort (nominal, DkW) 
  # and plot below has blue prediction line in the effort  
  # instead of the catch plot, mostly flat because of the different scales (TESTED so OK) 
  #
  # 1  1950 NomActive 60007404. Effort (nominal, DkW)
  # 2  1950 catch_tot   165558. Catch (tonnes)       
  # 3  1950 pred         61453. Effort (nominal, DkW)
  
  ## LME 1 - effort based predictions - with bug
  # Year Type          Value group                
  # <dbl> <chr>     <dbl[1d]> <chr>                
  # 1  1950 NomActive 44585112. Effort (nominal, DkW)
  # 2  1950 pred      38733547. Effort (nominal, DkW)
  ...
  # # test adding of data in WW years
  # head(final_df_plot)
  # final_df_plot %>% filter(Year >= 1938, Year <=1945)
  # final_df %>% filter(Year >= 1938, Year <=1945)
...
effort_spread<-function(historical_effort, recent_effort, toKeep, reference){
  ...
  # WARNING - problem with FAO_for_LME0_88 (antarctica) as there is no data until 1950 in original Yannick's dataset. 
  if(toKeep == "LME_Antarctica"){
    recent_effortB<-recent_effort %>% 
