...
## Load enviro data and do annual means *can skip depending on below* 
...
#### START LOOP for each LME - in parallel see code in 02!



#### not working as a function or a loop - try one by one 
...
Extract_LME_TimeSeries<-function(lme_mask_025, intpp_exp_annual, intpp_crt_annual){
  ...
  tic()
  intpp_exp_annual_lme <- mask(intpp_exp_annual, lme, updatevalue=NA)
  toc() # 15.622 sec elapsed
  # plot(intpp_exp_annual_lme[[50]]) 
  # dim(intpp_exp_annual_lme)

  tic()
  intpp_crt_annual_lme <- mask(intpp_crt_annual, lme, updatevalue=NA)
  toc() # 15.622 sec elapsed
...
# final plot 
  ...
  (plot<-ggplot(data = final, aes(x = Year, y = weighted_mean, group = Type, color = Type)) +
    ...
    ggtitle(paste("LME", i , lme_mask_name[[i]], sep = " "))+
    # facet_wrap(~group, scales = "free", nrow = 2)+
    annotate("rect",xmin=1850, xmax=1950, ymin=-Inf, ymax=Inf, fill = "#edf8fb", alpha = 0.4)+ # spin-up
...
# tic()
# a<-Extract_LME_TimeSeries(lme_mask_025[[22]], intpp_exp_annual = intpp_exp_annual, intpp_crt_annual = intpp_crt_annual)
# plot<-a$plot
# plot<-a$final
...
for(i in 1:2){
  ...
  a<-Extract_LME_TimeSeries(lme_mask_025[[25]], intpp_exp_annual = intpp_exp_annual, intpp_crt_annual = intpp_crt_annual)
  plot_new[[i]]<-a$plot
  final_new[[i]]<-a$final
...
# save data in case 
save.image(file = "~/FishingEffort/Data/temp_intpp_analysed.RData")
...
# try inpp one degree # NO need the 15arcmin ones to compare to control - but very slow so might be better onedegree as you develope the code - NOTE need to build the spin-up from control files and add them to experiment file!!!!! 
...
# #### SKYP this and do the below instead .... 
# # ARRIVATA QUI - need to speed this up and check for better functions 
...
# In .varName(nc, varname, warn = warn) : varname used is: mask_EBS
# If that is not correct, you can set it to one of: mask_EBS, mask_GOA, mask_CALC, mask_GOC, mask_GOM, mask_SEUSCS, mask_NEUSCS, mask_SCOT, mask_LAB, mask_IPH, mask_PCAC, mask_CARS, mask_HUMC, mask_PATC, mask_SBRS, mask_EBRS, mask_NBRS, mask_CEA, mask_GRLS, mask_BARS, mask_NORWS, mask_NS, mask_BS, mask_CBS, mask_IBC, mask_MEDS, mask_CRC, mask_GUIC, mask_BENC, mask_AGC, mask_SOMCC, mask_ARABS, mask_REDS, mask_BOB, mask_GOT, mask_SCS, mask_SULUS, mask_INDS, mask_NAS, mask_NEAS, mask_ECAS, mask_SEAS, mask_SWAS, mask_WCAS, mask_NWAS, mask_NZS, mask_ECS, mask_YELS, mask_KURC, mask_JAPS, mask_QYC, mask_OKS, mask_WBS, mask_NBS, mask_BEAS, mask_ESS, mask_LAPS, mask_KARS, mask_ISS, mask_FARP, mask_ANT, mask_BLS, mask_HUDB, mask_CARC, mask_ALEI, mask_CANHA
...
for(i in 1:length(lme_mask_name)){
  ...
  lme_mask_1[[i]]<-brick("/rd/gem/private/fishmip_inputs/ISIMIP3a/fishmip_regions/LMEs66_masks_0.25deg.nc", varname = lme_mask_name[[i]])
  names(lme_mask_1)[i]<-lme_mask_name[i]
  
}

dim(lme[[1]]) # why does this have a time dimention? is it OK?  
plot(lme[[1]])

# extrapolate 0.25 degree instead!  
...
# intpp
nc_data <- brick("/rd/gem/private/fishmip_inputs/ISIMIP3a/experiment/gfdl-mom6-cobalt2_obsclim_intpp_15arcmin_global_monthly_1961_2010.nc")
# dim(nc_data)
# class(nc_data)
# nc_data[,,600] # this is a brick object - not sure how to access data and dimention here ...  
# plot(nc_data[[600]]) # access rater layer 600
# class(nc_data)
# 
# # check and adjust?? same extent and crs?? 
# extent(nc_data)
...
# extact lme data only - this is where the LME loop will start .... 

# possible links
# https://stackoverflow.com/questions/51716315/how-to-subset-classify-raster-based-on-another-raster-grid-cells-values this should be the solution ... 
# focal does not seem the right function... 

# other approach 
# https://stackoverflow.com/questions/64785261/replacing-raster-values-with-another-raster-when-meeting-certain-condition
# dimension missing. both using brick and stack but in a different way...  

# SOLUTION!!!!! 
# crop won't work here as the extent of the lme file in terms of lat and long is the same as the intpp data. 
# x <- mask(nc_data, lme1, updatevalue=0)
# plot(x[[600]])
# dim(x)

x <- mask(nc_data, lme1, updatevalue=NA)
plot(x[[600]])
dim(x)
class(x)

# caclaulte annual means 

# extract data - this is to extract lme data from global raster - you already have done this above ... but this is useful to mean adn weight I guess... function need shape file as second object...  
# trial<-raster::extract(nc_data, lme1, fun = mean)

# trial<-calc(x, fun = mean)
# dim(trial)
# plot(trial)
# 
# trial<-stackApply(x, 1:12, mean) # this is monhlty mean we need yearly means 
# dim(trial)
# plot(trial)
# 
...
# calculate monthly means using stackApply
raster_mean <- stackApply(x, indices=indices, fun=mean, na.rm = TRUE) # WARNING - check this - also na.rm should be on? 
dim(raster_mean)
plot(raster_mean[[50]])
class(raster_mean)

# time series of weighted means 
# from here https://stackoverflow.com/questions/55230510/calculating-weighted-spatial-global-annual-averages-across-grid-cells-using-netc

# also weight by area available but example not working... 
# raster with latitude cell values 
w <- init(raster_mean, 'y')
# cosine after transforming to radians
w <- cos(w  * (pi/180))
# multiply weights with values
x <- raster_mean * w
# compute weighted average
weighted_mean<-cellStats(x, sum) / cellStats(w, sum)
df<-data.frame(Year = unique(indices), weighted_mean= weighted_mean)

# plot 
# WARNINNG - is this right? intpp increasing for this LME? need someone else to calcualte and check trends  
ggplot(df, aes(x = Year, y = weighted_mean))+
  geom_line()


### now do same for control and use the 1961-1980 on repeat to reconstruct spin-up for both files 


# end LOOP

#### need another loop for variable type (intpp, temp. phyto and zoo)
