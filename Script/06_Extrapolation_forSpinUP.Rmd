...
# not sure about above as this might have referred to historical only... 
...
# plot 
yMax=max(filter(trial3_toPlot, group == "Catch (tonnes)")$Value)
ann_text <- data.frame(Year = c(1850+5,1950+2,1961+15,2010+5,1850+20), Value = c(rep(yMax,4),yMax-50), lab = c("Spin-up","Transition","Experiment", "Validation","*Predictions taken from LME 65"), group = rep("Catch (tonnes)", 5), Data = rep("Reconstructed values", 5), Considered = rep("Yes", 5))
...
# if(df_kappa == 6){
#   name = "Output/reconstructed_effort_for_spinup_totLME2_JeromeComments_kappa6.pdf"
# }else if (df_kappa == 3){
#   name = "Output/reconstructed_effort_for_spinup_totLME2_JeromeComments_kappa3.pdf"
# }

# if total catch included discards: 
...
# pdf("Output/reconstructed_effort_for_spinup_JeromeComments.pdf", height = 8, width = 6)
# marrangeGrob(grobs = plot_reconstructed_effort_ref, nrow=2, ncol=1)
# dev.off()

# if total cacthes included discards 
...
head(reconstructed_effort_ref) 

yannick_dir <- "/rd/gem/private/users/yannickr"
# fwrite(x = reconstructed_effort_ref, file.path(yannick_dir, "effort_histsoc_1850_2017.csv"))
# if total cacthes included discards 
fwrite(x = reconstructed_effort_ref, file.path(yannick_dir, "effort_histsoc_1850_2017_discards.csv"))
