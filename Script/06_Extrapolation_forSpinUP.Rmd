...
catch_histsoc_1869_2017.csv (script 07) for catch data including both historical (before 1950) and recent (after 1950) data from Reg Watson.  
...
2. **reconstructed_effort_for_spinup.pdf** shows trends in fishing effort by gear (taken as an effort grouping example) and hence the distribution of the constructed effort for spin-up (1850-1950) across these groups.  

3. **effort_histsoc_1850_2017.csv** final spin-up and effort forcing data. 1850-1950 spin-up (extrapolated), 1950-1961 transition, 1962-2010 experiment, and 2011-2017 validation.     
...
# LME 40 not capturing slow increase in effort and resulting in high effort between 1850 and 1950, try playing with the smoothing parameter. 
kappa_values[c(40)+1, 2]<-3 # 4 and 5 won't make a change # 3 works

# LME 29 and 31 same as above - but I doubt this will change things as flat trend on the last years of data. 
kappa_values[c(29)+1, 2]<-4 # 5 or 7 means increases # 4 works 
kappa_values[c(31)+1, 2]<-5 # 5 works 
...
plot_reconstructed_effort_LME_war_ref[[27]]

# adjust LME 64 where effort increases back in time - use decreases as per LME 65
plot_reconstructed_effort_LME_war_ref[[65]] # the one need fixing 
plot_reconstructed_effort_LME_war_ref[[66]]
trial<-reconstructed_effort_LME_war_ref[[65]] # the one need fixing 
trial1<-reconstructed_effort_LME_war_ref[[66]]

# use the rate of change in trial1 to calculate trial (1850-1950)
...
trial2<-trial %>% 
  filter(Year >= 1950, Type %in% c("NomActive", "catch_tot")) # keep only catch (no historical available) and effort. Predictions are wrong and should be the effort one 
...
trial3$Considered <- "No" # none of these numbers were considered as the projections are taken from LME 65

# plot 
yMax=max(filter(trial3, group == "Catch (tonnes)")$Value)
ann_text <- data.frame(Year = c(1850+5,1950+2,1961+15,2010+5,1850+17), Value = c(rep(yMax,4),yMax-50), lab = c("Spin-up","Transition","Experiment", "Validation","*Predictions taken from LME 65"), group = rep("Catch (tonnes)", 5), Data = rep("Reconstructed values", 5), Considered = rep("Yes", 5))
    
plot<-ggplot(data = trial3, aes(x = Year, y = Value, group = Data, color = Data)) + # shape=Considered
  ...
  annotate("rect",xmin=2010, xmax=2017, ymin=0, ymax=Inf, fill = "#238b45", alpha = 0.4)+ # validation
  geom_text(data = ann_text,label = ann_text$lab, color = "Black", size = 2)+
  geom_point(data = filter(trial3, Data == "Reconstructed values"),size=1)+
  geom_line(data = filter(trial3, Data == "GAM predictions"))+
  scale_color_manual(values = c("blue", "black"), drop = FALSE)+
  scale_fill_chris()+
  my_theme+
  geom_vline(xintercept=1945, linetype="dashed", color = "red", size=0.5)+
...
plot_reconstructed_effort_LME_war_ref[[65]] <-plot
...
ggsave("Output/reconstructed_effort_for_spinup_totLME2_JeromeComments.pdf", marrangeGrob(grobs = plot_reconstructed_effort_LME_war_ref, nrow=1, ncol=2), device = "pdf")
...
plot_reconstructed_effort_ref[[27]]
...
# 
# 1. N, P and GT were given as part of the original effort file but are not reconstructed so NA for spin-up. Delete these columns and maybe provide them as separate file, done
...
# 3. For LME 64 effort is increasing by def. Use LME 66 and apply that rate of change. Find out which is Arctic and make sure taht LME is 0 untill 1950 - new fishery... TO DO 
# 4. Spatial grid cell effort - should we reconstruct that too? if so, should we use the globla trend and allocate effort actoss cells based on effort 5. contribution in 1950 as done for the LME aggregated data? TO DO  
