...
### SAVE DATA HERE as you need to create a plot of effort and gfdl intpp for the north sea in plotGFDLinputs 
# save.image(file = "~/FishingEffort/Data/temp_effort.RData")
...
# # but in some EEZ there are multiple LME and that could be a problem... 
...
# check eez
# most evidently problematic - kenia (in LME 31 and LME0)
...
data_plot_trial<-reconstructed_effort_ref_all %>% 
  filter(LME == nLME) %>% 
  # filter(eez_country_name %in% c("Cuba", "Haiti","Venezuela")) %>% 
...
plot<-ggplot(data = data_plot_trial, aes(x = Year, y = NomActive, group = eez_country_name, color = eez_country_name)) +
  ...
  geom_vline(xintercept=1960, linetype="dashed", color = "red", size=0.5)+
  my_theme
  # facet_wrap(~eez_country_name, scale = "free")

# pdf("Output/Angola.pdf", height = 10, width = 12) # height = 8, width = 6)
# plot
# dev.off()
