...
## Could this be problematic - e.g. coastal effort considered for LMEs and not EEZs? Asked Julia - only a few LME/EEZ at high lat affected. also these are the files used to work with catch and effort by Reg and Yannick and here we are using these files for consistency. 
# should we use the FAO_LME and FAO_EEZ interaction masks provided by Denisse (+FAO regions column) instead of LME and EEZ columns? No, these were created for inputs only + see reason above. 
...
join_effort_data <- function(this_file_name){
  ...
  # CN this version considers EEZ aggregation using EEZ FAOname instead of the administrative country name (Yannick's file) and intersection between LME and FAO regions to split the high seas. NO NEED AS the original code already does this
  # this_destination_path <- paste0("/rd/gem/private/users/yannickr/effort_mapped_by_country_aggregated_HighSea/", "aggregated_HighSea_", this_file_name)
  ...
  # if(file.exists(this_destination_path)){ # CN - why this? the file does not exist
    ...
    these_aggregated_data <-
      ...
      group_by(eez_country_name, fao_area, LME, LME_FAO, SAUP, Gear, FGroup, Sector) %>%
...
# CN consider the HighSea version as explained above - no need
# newly_written_files <- list.files(file.path(yannick_dir, "effort_mapped_by_country_aggregated_HighSea"), full.names = TRUE)
...
# # CN consider the HighSea version as explained above 
# fwrite(x = combined_aggregated_effort, file.path(yannick_dir, "all_effort_aggregated_HighSea.csv"))
