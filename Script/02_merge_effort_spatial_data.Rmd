...
## Could this be problematic - e.g. coastal effort considered for LMEs and not EEZs? Asked Julia - do trials below and plot outputs 
# should we use the FAO_LME and FAO_EEZ interaction masks provided by Denisse (+FAO regions column) instead of LME and EEZ columns? Use them both and check outputs. 
...
join_effort_data <- function(this_file_name){
  ...
  # CN this version consideres EEZ aggregation using EEZ FAOname instead of the administrative country name (Yannick's file) and intersection between LME and FAO regions to split the high seas. Need to add the intersection between EEZ and FAO regions once the above is solved
  ...
  # if(file.exists(this_destination_path)){ # CN - why this? the file does not exist
    ...
    these_aggregated_data <-
      ...
      left_join(FAO_LME_new, by = c("Lat", "Lon")) %>% # CN added 
