...
# load reconstructure effort data (temp_data) printed in 07_PHASE1 
...
for(i in 1:length(decision)){
  ...
  # i = 1 
  # WARNING problems with FAO_for_LME0_88 # i = 17 
  # WARNING problems with FAO_for_LME0_21 # i =1 
...
### SAVE DATA HERE for FAO report plots. NOTE here saved in /rd/gem as it should be for all temp_data!!!!!!! move them all here when re-running the code. 
save(reconstructed_effort_ref_all, file = "/rd/gem/private/users/camillan/FishingEffort_Data/temp_effort_EEZ.RData")
...
# effort allocated to groups
load("/rd/gem/private/users/camillan/FishingEffort_Data/temp_effort_EEZ.RData")
...
# NA for fAO_LME column 
trial<-reconstructed_effort_ref_all %>% 
  filter(Year == 1951)
head(trial) # need to add this column when extrapolating effort or when allocating effort to groups taht is <1950. 

# check NAs for all spatial groupings 
sort(unique(check), na.last = FALSE) # NOTE! sort() removes NAs unless specified. 
sort(unique(reconstructed_effort_ref_all$eez_country_name), na.last = FALSE) # There are no EEZ that are NAs! but there are EEZ that are high seas which is OK
sort(unique(reconstructed_effort_ref_all$LME), na.last = FALSE) # No NAs but LME 0 which is OK 
sort(unique(reconstructed_effort_ref_all$fao_area), na.last = FALSE) # there is an NA which is not good.  
trial<-reconstructed_effort_ref_all %>% 
...
FAO_check<-FAO_check %>% 
  filter(!is.na(fao_area)) %>% 
  summarise(e = sum(e))
# 995466621430. # when NA is removed 
# 995471507451. # otherwise 

# check range of values: 
# WARNING negative values.... 
min(reconstructed_effort_ref_all$NomActive) # -1.186503e-09 NEGATIVE! 
...
# check previously printed file
a<-read_csv("/rd/gem/private/users/yannickr/effort_histsoc_1841_2017_EEZ.csv")
head(a)
min(a$NomActive) # Here it is 0 so OK!

# check effort before extrapolation (printed at the end of 05 and after a simple merging)
b<-read_csv("/rd/gem/private/users/yannickr/effort_histsoc_1950_2017_EEZ_addFAO.csv")
min(b$NomActive) # Here it is 0 so OK! then this is happening when extrapolating effort in 07 phase 1.  

i = 1
a<-decision[[i]] # created in phase 1 ... 
min(a$Value, na.rm = TRUE)
filter(a, is.na(Value))
...
a2[order(a2$e, decreasing = TRUE), ] # very similar values but not exactly the same (possibly due to War bug correction in this version). 
...
# check what you'd have to do for aggregation aims 
# if using EEZ: 
c<-reconstructed_effort_ref_all %>% 
  filter(eez_country_name == "High Seas")
rownames(c)<-NULL
head(c)
...
a<-read_csv("/rd/gem/private/users/yannickr/DKRZ_EffortFiles/effort_histsoc_1841_2010_EEZ.csv")
