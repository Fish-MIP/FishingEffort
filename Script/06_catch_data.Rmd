...
(EEZ_adj <- bind_rows(EEZ %>% 
            ...
            take_first) %>%
    # select(Lat, Lon, A_Code, Admin_Country) %>% # changing to EEZ instead of administrative country
    # dplyr::rename(eez_country_code = A_Code, 
    #        eez_country_name = Admin_Country) %>% 
    select(Lat, Lon, CNumber, FAOname) %>% 
    dplyr::rename(eez_country_code = CNumber,
...
EEZ_adj<-EEZ_adj %>% select(-eez_country_code) # CN no need for this enymore - avoid confuction
