...
author: "Camilla Novaglio (revised by Gage Clawson)"
date: "November 2024"
output: 
  html_document:
    toc: yes
...
gem_dir <- file.path("/home/ubuntu/gem") # change to /rd/gem if that is the filepath for you 
...
effort <- fread(file.path(gem_dir, "private/users/yannickr/effort_histsoc_1950_2017_regional_model.csv"))
...
catch <- fread(file.path(gem_dir, "private/users/yannickr/catch_histsoc_1869_2017_regional_models.csv"))
catch <- catch %>% 
  filter(!is.na(region))
...
if(df_kappa == 6){
  name = "Output/new/reconstructed_effort_for_spinup_tot_regional_models.pdf"
}else if (df_kappa == 3){
  name = "Output/new/reconstructed_effort_for_spinup_tot_regional_models_kappa3.pdf"
}
...
effort_spread <- function(historical_effort, recent_effort, toKeep, reference){
  ...
  # # # trial
  # historical_effort = decision[[32]] # effort from 1841 to 2017 at LME level from analysis above - missing groups (eez, gear, fgroup, saup, sector, etc.)
  ...
  ## NOTE: Some regions (like Ross Sea) don't have any fishing until ~1970. When this is the case, we just skip it and don't reconstruct the effort the same way, since we don't have any data to inform it. 
...
toExpend <- toExpend %>% 
  tibble::add_row(region = toKeep, SAUP = NA_integer_, Gear = NA_character_,
                  FGroup = NA_character_, Sector = NA_character_, contribution = NA_real_)
  ...
  #   ungroup()
  # 
  # pdf("Output/new/new/trial.pdf", height = 8, width = 6)
  ...
  # pdf("Output/new/new/trial1.pdf", height = 8, width = 6)
  # plot
  # dev.off()
...
pdf("Output/new/reconstructed_effort_for_spinup_regional_models.pdf", height = 8, width = 6)
...
fwrite(x = reconstructed_effort_ref_all, file.path(gem_dir, "private/users/yannickr/effort_histsoc_1841_2017_regional_models.csv"))
