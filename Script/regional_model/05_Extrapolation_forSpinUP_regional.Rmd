...
toKeep<-sort(unique(effort$model_area_name))
...
effort_extrapolate<-function(toKeep, kappa, war_param, war_decision, reference){
  ...
  effort_extr<-effort %>% 
    filter(model_area_name %in% toKeep) %>% 
    group_by(Year) %>% 
...
# POSSIBLE WARNINGS - due to kappa  
...
# MIGHT NEED TO ADJUST SPECIFIC REGION MANUALLY AS PER GLOBAL ANALYSIS (LME-SCALE)
...
for(i in 1:length(decision)){
  ...
  recent_effort<-filter(effort, model_area_name == toKeep [[i]])
