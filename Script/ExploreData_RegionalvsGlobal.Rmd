...
ggplot(effort_mapped, aes(x = Year, y = effort))+
  geom_line()+
...
# Split effort by LME 
effort_list<-split(effort_global, effort_global$LME)

# Select South East Australia = LME 42
effort_global<-effort_list$'42'

# Select the fishing sector you want to focus on, unless you are considering all sectors
ggplot(effort_global, aes(x = Year, y = NomActive, group = Sector, color = Sector))+
  geom_point(size=0.5)+
  geom_line()+
  theme_bw()+
  facet_wrap(~Fleet)
effort_global<-effort_global %>% filter(Sector == "I")

# WARNING - outliers?  
# effort_global<-effort_global %>% filter(NomActive < 3e+07)

# Change columns names - more informative 
colnames(effort_global)<-c("LME","Year","Sector","Fleet" ,"Global_effort_DayKW", "Zone")
